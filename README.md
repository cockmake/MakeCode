...
- **模型配置**：通过内置的 `/models` 命令进行管理（详见 2.14 节）
...
### 2.12 子智能体 Todo 工具（`tools/todo.py`）

子智能体内部可使用 `TodoUpdate` 工具维护一个简易待办列表，用于多步骤任务跟踪。

### 2.13 MCP 服务集成（`utils/mcp_manager.py`） 🆕
...
### 2.14 模型管理面板（`system/models.py`） 🆕

MakeCode 提供可视化的模型配置管理功能，支持多模型切换和持久化存储。

#### 核心功能

- **磁盘持久化**：模型配置自动保存到 `.makecode/model_config.json`，跨会话保留
...
### 2.15 控制台渲染与输出优化（`system/console_render.py`） 🆕
...
- **流式输出支持**：支持实时流式渲染，适配 LLM 响应的增量输出场景
